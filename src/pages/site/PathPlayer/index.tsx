...
const PathPlayer = () => {
  ...
  const { courseId: courseIdFromParams } = cerficiateParams;

  // const { data: discussData, isFetching: isFetchingDiscuss } = useGetDiscussionsByCourseIdQuery(courseId);
  const { data: discussData, isFetching: isFetchingDiscuss } = useGetDiscussionsByCourseIdQuery(courseIdFromParams);
  
  // const { data: NotesData, isFetching: isFetchingNotesData } = useGetNotesByLessonIdQuery(lessonId, {
  //   skip: !lessonId
  // });

  const { data: NotesData, isFetching: isFetchingNotesData } = useGetNotesByUserIdQuery(userId, {
    skip: !userId
  });
  ...
  const tabItems: TabsProps['items'] = [
    ...
    {
      key: 'discuss',
      label: `${t('player.discuss')} (${discussData?.discuss?.length || 0})`, //Discuss
      children: <Discusses className='path-player__menu-content' />
    },
    {
      key: 'learners',
      label: `${t('player.learners')} (${usersData?.users?.length || 0})`, //Learners
      children: <Learners className='path-player__menu-content' />
    },
    {
      key: 'note',
      label: `${t('player.note')} (${NotesData?.notes?.length || 0})`, //Notes
              ...
              <div className='path-player__menu-header'>
                <div className='path-player__menu-header-nav'>
                  <Link className='path-player__menu-header-nav-back' to='/start'>
                    <ArrowLeftOutlined className='path-player__menu-header-nav-back-icon' /> {t('player.back')} {/* Back to course page */}
                  </Link>
                </div>
                <h3 className='path-player__menu-header-title'>{data?.course.name}</h3>
                <div className='path-player__menu-progress'>
                  <Progress percent={parseFloat((currProgress as unknown as number).toFixed(1))} status='active' />
                </div>
              </div>
              {/* Menu Content  */}
            ...
            <div className='path-player__player'>
              <div className='path-player__player-nav'>
                <div className='path-player__player-nav-item' onClick={handlePreviousClick}>
                  <LeftOutlined /> {t('player.previous')} {/* Previous */}
                </div>
                <div className='path-player__player-nav-item' onClick={handleNextClick}>
                {t('player.next')} <RightOutlined /> {/* Next */}
                </div>
              </div>
