...
import { useDeleteDiscussionMutation, useGetDiscussionsByLessonIdQuery, useGetDiscussionsByCourseIdQuery, useGetUserDetailQuery } from '../../../../../client.service';
...
const CommentList: React.FC<CommentsProps> = ({ userId, lessonId, courseId }) => {
  ...
  const { data, isLoading, isError, refetch } = useGetDiscussionsByCourseIdQuery(courseId);
  const discuss = data?.discuss;
  const [isUpdateModalVisible, setIsUpdateModalVisible] = useState(false);
  const [updatingId, setUpdatingId] = useState<string | null>(null);
  const [isReplyModalVisible, setIsReplyModalVisible] = useState(false);
  const [parentDiscussId, setParentDiscussId] = useState<string>('');
  const [discussId, setDiscussId] = useState<string>('');
          ...
          renderItem={(item) => (
            <List.Item>
              <List.Item.Meta
                ...
                avatar={<Avatar src={typeof item.userId.avatar === 'string' ? item.userId.avatar : undefined} />}
