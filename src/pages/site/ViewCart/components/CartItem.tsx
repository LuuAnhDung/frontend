  ...
  return (
    <Fragment>
      <div className='view-cart__list-item'>
        ...
        <Row>
          <Col md={4}>
            <Link to={`/courses/course/detail/${_id}`} className='view-cart__list-item-thumb'>
              <img src={thumbnailUrl} alt='' className='view-cart__list-item-img' />
            </Link>
          </Col>
          <Col md={12}>
            <div className='view-cart__list-item-info'>
              <Link to={`/courses/course/detail/${_id}`} className='view-cart__list-item-info-line view-cart__list-item-info-name'>
                {name}
              </Link>
              <p className='view-cart__list-item-info-line view-cart__list-item-info-author'>
                {/* By <Link to={`/user/${props.courseItem.userId?._id ?? ""}`}>{props.courseItem.userId?.name}</Link> */}
                By {props.courseItem.userId?.name ?? ""}
              </p>
