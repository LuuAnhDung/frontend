...
const ReviewItem: React.FC<{ review: IReview }> = ({ review }) => {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <Avatar 
          src={
            typeof review.createdBy === 'string' 
              ? 'N/A' 
              : review.createdBy?.avatar instanceof File 
              ? URL.createObjectURL(review.createdBy?.avatar) 
          ...
          <div style={{ marginTop: '30px', marginLeft: '30px' }}>
            {repliesData.replies.map((reply) => (
              <div key={reply._id} style={{ display: 'flex', marginBottom: 10, alignItems: 'flex-start' }}>
                <Avatar 
                  src={
                    typeof reply.createdBy === 'string' 
                      ? 'N/A' 
                      : reply.createdBy?.avatar instanceof File 
                      ? URL.createObjectURL(reply.createdBy.avatar) 
                      : reply.createdBy?.avatar || 'N/A'
                  }
                />
                <div style={{ marginLeft: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', marginBottom: 5 }}>
