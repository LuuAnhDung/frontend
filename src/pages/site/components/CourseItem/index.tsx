...
const CourseItem = (props: CourseItemProps) => {
  ...
  const { t, i18n } = useTranslation('courseitem'); 
  ...
  const btnClickHandler = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (isAuth) {
      if (dataAction === 'buynow') {
        ...
        if (!props.courseItem) {
          notification.error({ message: 'Course information is missing. Please try again.' });
          return;
        }
        ...
        if (newOrderItem) {
          ...
          notification.success({ message: 'You have successfully enrolled in the course!' });
        }
      }
    } else {
                ...
                <Col md={12}>
                  {!hasBought && props.courseState !== 'ordered' && (
                    ...
                    >
                      ...
                      {props.courseState !== 'ordered' && (props.courseItem.finalPrice === 0 ? t('courseitem.enroll') : t('courseitem.buy'))}
                    </Button>
                  )}
                  {hasBought && (
                    <Button
                  ...
                  {props.courseState !== 'ordered' && (
                    <div className='course-item__prices'>
                      {props.courseItem.finalPrice === 0 ? (
                        <div className='course-item__prices-free'>{t('courseitem.free')}</div> //FREE
                      ) : (
                        <>
