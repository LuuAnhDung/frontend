...
import { useUpdateActiveStatusCourseMutation, useGetAllCoursesQuery, useGetCoursesQuery, useDeleteCourseMutation } from './course.service';
...
const Courses = () => {
  ...
  const [deleteCourse, { isLoading, isSuccess, error }] = useDeleteCourseMutation();
    ...
    if (dataList) {
      const sourceCourseData = dataList.courses.map((courseItem) => {
        ...
        const courseTemplateItem: DataCourseType = {
          ...
          author: userId?.name ?? 'Default Author',
    ...
    if (allCoursesData) {
      const sourceCourseData = allCoursesData.courses.map((courseItem) => {
        ...
        const courseTemplateItem: DataCourseType = {
          ...
          actions: (
            <Space size='small'>
              <Button icon={<EditOutlined style={{ color: '#1890ff' }} />} onClick={() => handleUpdate(_id ?? "default-id")} />
              <Button icon={<EyeOutlined style={{ color: '#1890ff' }} />} onClick={() => handleViewDetails(_id ?? "default-id")} />
              ...
              {/* {isDeleted ? (
                ...
                <Popconfirm
                  title='Are you sure you want to delete this course?'
                  placement='topRight'
                  onConfirm={() => handleDelete(_id ?? "default-id")}
                  okText='Yes'
                  cancelText='No'
                >
